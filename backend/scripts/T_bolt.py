...
Stepped Tube with Octagonal Boss. Can be run standalone or called from API.

Usage:
    python T_bolt.py --thread-size M10 --length 170 --head-width 76 --head-height 25 --slot-width 8 --thread-length 37
    python T_bolt.py --output custom_tbolt.glb --thread-size M12 --length 200 ...
"""
...
# ============================================================================
# DYNAMIC SCRIPT TEMPLATE - Full T-Bolt geometry
# ============================================================================
SCRIPT_TEMPLATE = '''
"""
...
def generate_script(output_path, output_format, thread_size, length, head_width, head_height, slot_width, thread_length):
    ...
    if isinstance(thread_size, str) and thread_size.upper().startswith('M'):
        try:
            thread_diameter = float(thread_size[1:])
        except:
            thread_diameter = 10.0
    else:
        try:
            thread_diameter = float(thread_size)
        except:
            thread_diameter = 10.0
    
    # Calculate derived parameters
    inner_bore_radius = thread_diameter / 2.0        # Thread radius
    bore_axial_len    = max(10.0, length)            # Main body length
    ...
    outer_shelf_w     = max(5.0, thread_length)             # Thread/outer shelf
...
def main():
    ...
    parser = argparse.ArgumentParser(description="FreeCAD T-Bolt Generator with Custom Dimensions")
    ...
    # T-Bolt Dimensions
    parser.add_argument("--thread-size", type=str, required=True, help="Thread Size (e.g., M10, M12, M20)")
    parser.add_argument("--length", type=float, required=True, help="Body Length (L) in mm")
    parser.add_argument("--head-width", type=float, required=True, help="Head Width (W) in mm")
    parser.add_argument("--head-height", type=float, required=True, help="Head Height (H) in mm")
    parser.add_argument("--slot-width", type=float, required=True, help="Slot Width (S) in mm")
    parser.add_argument("--thread-length", type=float, required=True, help="Thread Length (T) in mm")
    
    args = parser.parse_args()
    
    print("=" * 60)
    print("Generating T-Bolt with Dimensions:")
    print(f"  Thread Size:   {args.thread_size}")
    print(f"  Length:        {args.length} mm")
    print(f"  Head Width:    {args.head_width} mm")
    print(f"  Head Height:   {args.head_height} mm")
    print(f"  Slot Width:    {args.slot_width} mm")
    print(f"  Thread Length: {args.thread_length} mm")
    print("=" * 60)
    ...
    success, log = run_freecad_script(script, use_gui=args.gui)
    
    if success:
        print("Process complete.")
    else:
        print(f"Process failed: {log}")
        sys.exit(1)
